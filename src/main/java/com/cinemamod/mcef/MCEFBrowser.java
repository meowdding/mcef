...
import com.cinemamod.mcef.utils.MultiVersionHelper;
...
public class MCEFBrowser extends CefBrowserOsr {
    ...
    public void setCursor(CefCursorType cursorType) {
        if (cursorType == CefCursorType.NONE) {
            GLFW.glfwSetInputMode(MultiVersionHelper.handle(), GLFW_CURSOR, GLFW_CURSOR_HIDDEN);
        } else {
            GLFW.glfwSetInputMode(MultiVersionHelper.handle(), GLFW_CURSOR, GLFW_CURSOR_NORMAL);
            GLFW.glfwSetCursor(MultiVersionHelper.handle(), MCEF.getGLFWCursorHandle(cursorType));
        }
    }
}
